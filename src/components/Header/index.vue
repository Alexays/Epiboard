  ...
  <header tag="header" class="grey">
    ...
    <template v-if="$store.state.settings.global.header_design === 'full'">
      <a :href="typer.current.length > 0 ? 'https://www.google.com/#q=' + typer.current : null" v-show="$route.path === '/'">
        <vue-typer tag="h1" :text="messages" pre-erase-delay='5000' erase-delay='250' :shuffle='true' @erased='addTrends' @typed='onTyped'></vue-typer>
      </a>
    ...
    <v-toolbar v-else card absolute prominent>
      <v-text-field v-on:keyup.native="search" prepend-icon="search" :value="typer.part" @input="typer.typed = $event" hide-details single-line></v-text-field>
      <v-spacer></v-spacer>
