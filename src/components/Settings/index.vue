    ...
    <v-card class="container">
      ...
      <v-layout align-center>
        <v-autocomplete
          ...
          :label="$t('settings.choose.background')"
        />
        <v-text-field
        ...
        <div v-if="settings.header.background === 'local'" class="file-btn">
          ...
          <v-btn
            ...
            @click="$refs.inputLocal.click()"
          >{{ backgroundLocal.filename || $t('settings.browse') }}</v-btn>
          <v-btn v-show="backgroundLocal.filename" icon @click="deleteBackgroundLocal">
      ...
      <v-layout align-center>
        ...
        <v-autocomplete
          ...
          :label="$t('settings.choose.trends')"
        />
      </v-layout>
      ...
      <v-layout>
        ...
        <v-text-field
          ...
          :label="$t('settings.placeholder.color')"
          @change="themeChange"
        />
      </v-layout>
      <h4 v-t="'settings.choose.custom_font'" class="subtitle-1" />
      <v-layout align-center>
        <v-flex xs2>
          <v-switch
            v-model="settings.theme.customFont"
            :label="$tc('settings.onOff', settings.theme.customFont)"
            class="mt-0"
            hide-details
          />
        </v-flex>
      ...
      <template v-if="settings.debug">
        <h4 v-t="'settings.custom_css'" class="subtitle-1" />
      ...
      <v-layout align-center>
        ...
        >
          ...
          <v-time-picker
            v-model="settings.dark.from"
            format="24h"
            @change="$refs.menu_from.save(settings.dark.from)"
          />
        </v-menu>
        <v-menu
          ref="menu_to"
        ...
        >
          ...
          <v-time-picker
            v-model="settings.dark.to"
            format="24h"
            @change="$refs.menu_to.save(settings.dark.to)"
          />
        </v-menu>
      </v-layout>
      ...
      <v-switch
        :disabled="!$ga"
        :label="$tc('settings.onOff', settings.analytics)"
        v-model="settings.analytics"
      />
      <h4 class="headline">Debug</h4>
      <h4 v-t="'settings.debug_desc'" class="subtitle-1" />
      <v-switch :label="$tc('settings.onOff', settings.debug)" v-model="settings.debug" />
      <h4 v-t="'settings.whatsnew'" class="subtitle-1" />
      <v-switch
