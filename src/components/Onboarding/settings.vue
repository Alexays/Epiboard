<template>
  <v-card :width="500" hover raised dark class="primary foreground--text mx-auto mt-4">
    ...
    <v-card-text class="pt-4">
      ...
      <h4 v-t.preserve="'settings.choose.design'" class="subtitle-1" />
      ...
      <v-checkbox :label="$t('settings.whatsnew')" v-model="settings.whatsnew" hide-details />
    </v-card-text>
    <v-card-actions class="secondary">
      <v-btn
        v-t.preserve="'onboarding.previous'"
        color="teal lighten-4"
        text
        @click="$emit('prev')"
      />
      <v-spacer />
      <v-btn v-t.preserve="'onboarding.next'" color="teal lighten-4" text @click="$emit('next')" />
    </v-card-actions>
  </v-card>
</template>
<script>
