  ...
  <div id="weather">
    <v-layout row wrap v-if="today">
        ...
        <v-layout column align-content-space-between fill-height>
          ...
          <v-flex>
            <v-layout row wrap class="main">
              <v-flex>
                <img :title="today.weather[0].description" :src="'/static/img/weather/weather-'+ getImg(today.weather[0]['id']) +'.png'"/>
              </v-flex>
              <v-flex>
      ...
      <v-flex xs4 class="weather-info blue-grey white--text head-drag">
        <v-layout column>
          <v-flex class="detail">
            <v-icon medium color="white">opacity</v-icon> <span title="Humidity"><b>{{today.main.humidity}}</b>%</span>
          </v-flex>
          <v-flex class="detail">
            <v-icon medium color="white">wrap_text</v-icon> <span title="Wind speed"><b>{{today.wind.speed}}</b> <span class="unit">km/h</span></span>
          </v-flex>
          <v-flex class="detail">
